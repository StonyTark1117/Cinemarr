...
The in-world decoder/renderer, control UI, subtitle/audio-track selection, positional sound, persistence/reconnect behavior, server-owned HLS relay, and clean protocol rejection paths are implemented. On 2026-08-27, every one of the 21 listed runtime profiles built the gate screen through its real 144p Quick TV block and proved the persisted 16x9 geometry, 256x144 rendition, and owner. Every two-client pair visibly rendered the identifiable synthetic H.264/AAC program, produced synchronized audible output, exercised the full HLS path, and stopped cleanly. Modern clients measure the OpenAL source cursor and physical output latency during a silent preroll, then compensate the queued audio so the first program sample reaches each sink at the shared media boundary. Representative post-compensation Fabric 1.21.1, NeoForge 1.21.1, NeoForge 26.1.2, and NeoForge 26.2 captures remained within 10–40 ms. Forge 1.7.10 passed consecutive fresh captures after its LWJGL 2 adapter began confirming physical backend progress inside a silent preroll and feeding larger buffers; the runs measured 0.986105 correlation at -10 ms and 0.978383 at -30 ms. The matrix includes a follower-first run that proves an already-watching client receives a television constructed later. Exact results are recorded in [release acceptance](docs/RELEASE_ACCEPTANCE.md).
