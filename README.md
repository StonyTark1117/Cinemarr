...
Protocol-10 code commit `1cbd341` has passed all 21 maintained fake-Plex runtime profiles, all 10 required GameTests, deep inspection of all 16 canonical artifacts, byte-identical rebuild checks, expanded adverse-network gates, generic endpoint hygiene, clean teardown, and fully green GitHub Actions run `33392358275`. Its hosted 16-JAR bundle passed checksums and inspection and matches the local release index byte-for-byte. Later operational-only commits make DiscPanel artifact deployment fail closed and narrowly retry identified upstream dependency-download failures without changing packaged mod bytes. Cinemarr remains a prerelease because the exact candidate still needs credentialed real-Plex, recovery, lifecycle, configured-secret, and packaged-native recertification, followed by green CI and hosted-bundle parity for the final documentation SHA. Earlier real-Plex and native evidence remains useful regression history but does not independently promote later bytes. See [the 1.0 hardening plan](docs/1.0_RELEASE_HARDENING_PLAN.md) and [the post-1.0 Jammarr target-feasibility assessment](docs/JAMMARR_TARGET_FEASIBILITY.md).
