...
candidate acceptance. The subsequent 838-input frozen build pair passed with
all eighteen bundle files identical, all sixteen artifact inspections and ten
explicit GameTests in each build. Formal Windows run `20260909T035031Z` then
passed exact review of all 279 used inputs, three fixtures and 69 Windows native
entries across all sixteen artifacts. Its retained guest and host run resources
were closed. That source is now superseded by the legacy defect correction
described above. Evidence remains under
`build/release-resume-20260909/native-payload-identity/`. No release candidate,
final-commit gate, real-Plex/ARM certification or green final-SHA CI is claimed.

## Current certification boundary — 2026-09-09 UTC

**Prerelease under hardening; no new-device candidate certification.** Resumption
began at clean `d967a43de5a6f43115018ba0c8189b2c7f16d12d`; previous ignored local
evidence is not present in this checkout. Its
[handoff CI run](https://github.com/StonyTark1117/Cinemarr/actions/runs/34289433197)
failed seven target jobs and skipped `1.0 release gate`. Five Fabric profiles
failed private-window lookup after mismatch rejection; legacy exhausted the
300-second program during post-reload stabilization; Forge 26.1.2 reported a
cancelled obsolete decode as a rejected segment during stream replacement.

Current corrective source adds a bounded normal-close window-discovery wait,
generation-owned decode completion and retirement, and a 900-second default
control fixture. It does not exempt active cancellation/failures, relax the
eight-second physical-A/V requirement, or accept forced client exits. Fourteen
private-window unit tests and five real-X test methods pass; the latter include
delayed mapping and exit-status-only native crashes. Core: 204 total, 203 passed,
one skipped. Root: 40 total, 38 passed, two skipped. The three new core tests
exercise stale results, active failures, concurrent publication/close and
permanent retirement. These are deterministic checks only.

Evidence is under `build/release-resume-20260909/`: the handoff input manifest,
1,358 hashed downloaded diagnostics, initial missing-Xvfb tooling failure,
22-task successful tooling preflight, window-wait red/green checks, real-X
checks, corrective Java test log and a fresh retained-native ready/off audit.
An initial full diagnostic build was deliberately interrupted for the fixes;
its receipt is preserved. The first frozen-source full build passed in 1,647
seconds: all 89 tasks executed, all ten GameTests passed, all sixteen artifacts
inspected, and 835 non-documentation inputs unchanged. Its eighteen-file bundle
is retained as `candidate-bundle-1`. Its second build was deliberately stopped
after 278 seconds for a client-environment isolation defect found during review.
The real-Plex wrapper exported Plex and DiscPanel credentials into the client
process environment. Shell client boundaries and the packaged Java probe/launch
now remove `CINEMARR_PLEX_TOKEN`, `CINEMARR_PLEX_URL`, `DISCOPANEL_TOKEN` and
`DISCOPANEL_API_BASE`, preserving the parent harness and required audio settings.
Both new canary regressions failed before the fix; five client-exit and 21
packaged-input tests now pass. The original first build is not relabelled.

Fresh verification under `credential-isolation/` passed all 22 tooling tasks
and two forced full builds in 1,384 and 1,331 seconds. Each executed all 89
tasks, inspected all sixteen artifacts and explicitly passed all ten required
GameTests. All sixteen JARs and both manifests match exactly across the two
bundles; all 835 frozen non-documentation inputs remained unchanged. The build
service exited zero with no recorded core dumps. Second-pass reports contain
core 204 total/203 passed/1 skipped, root 40/38/2 and legacy 249/247/2; suites
overlap. Four exact-bundle packaged-client preflights passed without launching
clients. Runtime preparation passed after all five Fabric asset warms and the
NeoForge asset warm (sixteen network workers), followed by decoder-benchmark
bundle preparation. Two earlier cold-cache preparation attempts were
deliberately interrupted and preserved; neither was a runtime case. The serial
37-case runtime batch has started. Legacy terminal is the first scoped accepted
case: all 32 gate images directly reviewed, five independent eight-second PCM
pairs at 10–20 ms with correlations 0.988554–0.999412, six quiet-state checks,
six verified zero client exits, and closed private displays/audio sinks/game
ports with no owned-unit core dump. Both world returns and replay show moving
video on both clients; away captures show the follower in the Nether. Queue,
near-EOS replacement, idle, replay and stop complete. This is 1 of 37 runtime
cases, not full matrix, real-Plex, native decoder or final-SHA certification.

Modern terminal is also scoped accepted after all 18 original image reviews,
three independent eight-second PCM pairs at 0/10/20 ms (correlations
0.999972/0.867982/0.989621), four quiet checks, five actual zero client exits,
and closed owned displays/sinks/ports with no unit core dump. Join toasts
partly cover the initial TV captures; all queue/replay sequences show advancing
video on both clients without that obstruction. A source/log audit classifies
one raw read-exhaustion event per original stream in the last three seconds
as terminal input drain with OpenAL prefetch, not an active underrun; the active
counter remains zero. Legacy pressure is also scoped accepted: all 75 original
images reviewed, five independent eight-second PCM pairs at 20–30 ms
(correlations 0.987536–0.999201), silent network-only peer, seven normal exits,
and clean owned resources. The main PCM and six final world images cover
reconnect and both resource/sound reloads per client. The paused control PCM is
overwritten by resumed audio in the maintained harness; pause silence is an
automated assertion only. Two held-browse BrokenPipe responses are preserved
as timed-out metadata connections. Modern pressure is the fourth scoped accepted case: all 69 originals
...
claim is made. This is 4 of 37 cases; legacy fault recovery is running. These remain scoped results for the frozen candidate. See the plan's new-device checkpoint for remaining
requirements; every original phase remains in scope.

## Historical certification boundary — 2026-09-08 UTC
