...
Protocol 11 adds display settings and separate watch-party/TV stream identities. Upgrade the server and all clients together; protocol 10 clients cannot join a protocol 11 server. The integrated implementation remains under hardening and requires fresh release acceptance.

The earlier protocol 10 migration removed the inherited global music queue, stations, MP3 streaming, and music UI. Old music-only configuration keys are ignored and dropped when the canonical server file is rewritten. Old music saved data is not used by Cinemarr 1.0. Television geometry, ownership, presentation, rendition, named-session, and video queue/checkpoint data remain the supported persistence surface.
