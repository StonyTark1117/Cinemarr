...
| Platform matrix | Code commit `1cbd341` passed the complete local 16-artifact/21-runtime matrix, all 10 GameTests, and fully green hosted CI run `33392358275` with bundle parity; later run `33398719853` passed 15 artifact/runtime jobs but correctly failed 1.21.1 Fabric A/V | Obtain a fully green run for the final SHA and preserve exact bundle parity |
...
| Release evidence | Full fake-Plex, GameTest, artifact, reproducibility, hygiene, hosted-CI, and bundle-parity evidence remains green for `1cbd341`; `21431bf` locally repairs the later hosted 1.21.1 Fabric A/V failure, while the attempted exact external run stopped before opening clients when DiscPanel failed a rollback upload/server update and its credential holder then exited | Complete exact real-Plex, recovery, lifecycle, configured-secret, and native recertification, then obtain green CI for the final documentation SHA |
