...
| Client video/audio | Implemented and matrix runtime-tested | One decoder/dynamic texture per visible session, batched masked screen rendering, positional audio from the nearest screen point, clock filtering, and drift correction. Modern clients map the shared media position to a server-owned epoch and compensate measured OpenAL physical output latency inside a silent preroll; representative final-implementation captures stayed within 10–70 ms. All 21 profiles have deterministic two-client visual/audio evidence; Forge 1.7.10 passed consecutive fresh physical captures after its LWJGL 2 adapter began confirming backend progress inside a silent preroll. |
