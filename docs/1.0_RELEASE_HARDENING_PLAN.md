...
Cinemarr remains a prerelease until the current scoped changes are committed, pushed, and the exact pushed SHA passes the complete GitHub matrix plus hosted-bundle parity. The current working tree has passed all 16 artifact targets, all 10 GameTests, deep inspection, exact four-profile DiscPanel deployment, real-Plex playback, recovery, lifecycle, live-value secret scanning, reusable Windows/ARM native decoding, and a fresh 21-runtime matrix. Final hosted CI is the remaining boundary; historical green runs are not substitutes.
