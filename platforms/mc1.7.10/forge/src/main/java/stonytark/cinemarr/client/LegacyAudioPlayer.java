...
import stonytark.cinemarr.core.client.PcmGain;
...
final class LegacyAudioPlayer {
    private static final String SOURCE = "cinemarr:global_music";
    ...
    void chunk(TransportPackets.AudioChunk value) {
        ...
        if (receivedChunks++ == 0) Cinemarr.LOGGER.info("Cinemarr legacy client received the first audio chunk");
    ...
    void tick() {
        ...
        if (CinemarrSettings.enabled()) LegacySoundAccess.stopVanillaMusic(minecraft);
        ...
        if (!started && !manifest.paused() && CinemarrSettings.enabled() && decoder.format() != null
    ...
    private boolean feedPcm(long now) {
        ...
        try {
            while (queuedUntilLocalMs - now < TARGET_SOUND_QUEUE_MS) {
                ...
                if (pcm == null) break;
                PcmGain.apply(pcm, CinemarrSettings.volume()
            ...
            Cinemarr.LOGGER.warn("Cinemarr legacy sound engine changed during PCM feed", unavailable);
    ...
    void listeningChanged() {
        if (!CinemarrSettings.enabled()) resetAudio();
    ...
    private void requestRebuffer(String reason) {
        ...
        Cinemarr.LOGGER.warn("Cinemarr legacy audio recovery attempt {}/{}: {}",
                recoveryAttempts, MAX_RECOVERY_ATTEMPTS, reason);
        if (stonytark.cinemarr.core.protocol.ProtocolLimits.audioProbeEnabled()) {
    ...
    void acceptanceExhaustRecovery() {
        if (!stonytark.cinemarr.core.protocol.ProtocolLimits.audioProbeEnabled() || manifest == null) return;
        recoveryAttempts = 0;
    ...
    String state() {
        if (!CinemarrSettings.enabled()) return "DISABLED";
    ...
    private void resetAudio() {
        ...
        if (previous != null) {
            try {
                ...
                Cinemarr.LOGGER.warn("Cinemarr legacy sound engine changed during cleanup", unavailable);
            }
        }
