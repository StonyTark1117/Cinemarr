...
final class LegacyStreamingMp3Decoder implements AutoCloseable {
    ...
    LegacyStreamingMp3Decoder(int firstChunk, int totalChunks) {
        ...
        thread = new Thread(new Runnable() { @Override public void run() { decode(); } }, "cinemarr-legacy-mp3-decoder");
    ...
    private void decode() {
        ...
        try {
            ...
            if (!closed) {
                ...
                Cinemarr.LOGGER.warn("Cinemarr legacy MP3 decoder stopped", error);
            }
        } finally {
            finished = true;
