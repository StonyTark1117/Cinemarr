...
/** Forge 1.7.10 ICommand surface matching the modern /cinemarr command tree. */
public final class LegacyCommands extends CommandBase {
    ...
    @Override public String getCommandUsage(ICommandSender sender) {
        return "/cinemarr [status|pause|resume|skip|clear|reload|cache|diagnostics|station|autoplay|adventure]";
    }
    ...
    public void processCommand(ICommandSender sender, String[] arguments) throws CommandException {
        ...
        if ("reload".equals(action)) { requireOperator(sender); coordinator.validatePlex(); reply(sender, "Cinemarr Plex validation started"); return; }
        if ("cache".equals(action)) { requireOperator(sender); reply(sender, "Cinemarr cache: " + coordinator.cacheSize() / 1024L / 1024L + " MiB"); return; }
    ...
    private void station(ICommandSender sender, String[] arguments) throws CommandException {
        ...
        } else throw new CommandException("Usage: /cinemarr station [status|stop|library-shuffle]");
    ...
    private void autoplay(ICommandSender sender, String[] arguments) throws CommandException {
        ...
        if (arguments.length != 2 || !("on".equalsIgnoreCase(arguments[1]) || "off".equalsIgnoreCase(arguments[1]))) {
            throw new CommandException("Usage: /cinemarr autoplay <on|off>");
        }
    ...
    private void adventure(ICommandSender sender, String[] arguments) throws CommandException {
        ...
        if (!"stop".equalsIgnoreCase(arguments[1])) {
            throw new CommandException("Usage: /cinemarr adventure [status|stop]");
        }
        coordinator.station(getCommandSenderAsPlayer(sender), new ControlPackets.StationRequest(
    ...
    private static void requireOperator(ICommandSender sender) throws CommandException {
        if (!sender.canCommandSenderUseCommand(CinemarrSettings.operatorPermissionLevel(), "cinemarr")) {
