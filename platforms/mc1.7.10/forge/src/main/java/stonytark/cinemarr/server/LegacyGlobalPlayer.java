...
public final class LegacyGlobalPlayer implements AutoCloseable, LegacyNetwork.ServerListener {
    ...
    public LegacyGlobalPlayer(final MinecraftServer server) throws IOException {
        delegate = new GlobalPlaybackCoordinator<EntityPlayerMP>(new CoordinatorRuntime<EntityPlayerMP>() {
            ...
            @Override public boolean isOperator(EntityPlayerMP player, int permissionLevel) {
                return player.canCommandSenderUseCommand(permissionLevel, "cinemarr");
            }
            ...
            @Override public java.nio.file.Path cacheDirectory() { return server.getFile("cinemarr-cache").toPath(); }
            @Override public void execute(Runnable action) { mainThreadActions.add(action); }
            @Override public void send(EntityPlayerMP player, CinemarrMessage message) { sendCore(player, message); }
    ...
    private static void sendCore(EntityPlayerMP player, CinemarrMessage message) {
        ...
        else throw new IllegalArgumentException("Unsupported Cinemarr clientbound message " + message.getClass().getName());
    }
