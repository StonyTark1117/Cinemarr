...
final class LegacyVideoRenderer {
    ...
    void render(LegacyVideoPlaybackManager playback, LegacyVideoClientState state) {
        ...
        for (VideoPackets.SessionState television : state.televisions()) {
            ...
            LegacyVideoTexture displayTexture=pipeline.texture().forDisplay(television);
            if(stonytark.cinemarr.core.client.DisplayFrameCapture.isSuppressed(television.controllerPos()))continue;
            displayTexture.bind(); MeshCache mesh = mesh(television);
