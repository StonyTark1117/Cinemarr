...
public final class CinemarrVideoScreen extends Screen {
    ...
    private void addControls(){
        ...
        widget("display-settings",Button.builder(Component.literal("Display"),b->minecraft.setScreen(new DisplaySettingsScreen(controllerPos,state,this))).bounds(layout.left(),6,68,20).build());
    ...
    private void saveAcceptanceScreenshot(){if(!acceptanceScreenshotPending||minecraft==null)return;acceptanceScreenshotPending=false;final stonytark.cinemarr.core.client.AtomicScreenshotFile screenshot = stonytark.cinemarr.core.client.AtomicScreenshotFile.create(minecraft.gameDirectory, "cinemarr-video-ui-acceptance.png");Screenshot.grab(minecraft.gameDirectory,screenshot.fileName(),minecraft.getMainRenderTarget(),message->Cinemarr.LOGGER.info("Acceptance video UI screenshot: {}",screenshot.publish(message.getString())));}
