...
public final class CinemarrCommands {
    ...
    @SubscribeEvent public void commands(RegisterCommandsEvent event) {
        ...
        root.then(Commands.literal("reload").requires(s -> s.hasPermission(CinemarrSettings.operatorPermissionLevel())).executes(c -> { GlobalPlayer p = CinemarrServer.instance().player(); if (p != null) p.validatePlex(); c.getSource().sendSuccess(() -> Component.literal("Cinemarr Plex validation started"), false); return 1; }));
        root.then(Commands.literal("cache").requires(s -> s.hasPermission(CinemarrSettings.operatorPermissionLevel())).executes(c -> { GlobalPlayer p = CinemarrServer.instance().player(); long bytes = p == null ? 0 : p.cacheSize(); c.getSource().sendSuccess(() -> Component.literal("Cinemarr cache: " + bytes / 1024 / 1024 + " MiB"), false); return 1; }));
        root.then(Commands.literal("diagnostics").requires(s -> s.hasPermission(CinemarrSettings.operatorPermissionLevel())).executes(c -> { GlobalPlayer p = CinemarrServer.instance().player(); c.getSource().sendSuccess(() -> Component.literal(p == null ? "Cinemarr is unavailable" : p.diagnostics()), false); return 1; }));
        root.then(Commands.literal("station").requires(s -> s.hasPermission(CinemarrSettings.operatorPermissionLevel()))
        ...
        root.then(Commands.literal("autoplay").requires(s -> s.hasPermission(CinemarrSettings.operatorPermissionLevel()))
        ...
        root.then(Commands.literal("adventure").requires(s -> s.hasPermission(CinemarrSettings.operatorPermissionLevel()))
                .then(Commands.literal("status").executes(c -> { GlobalPlayer p = CinemarrServer.instance().player(); c.getSource().sendSuccess(() -> Component.literal(p == null ? "Cinemarr is unavailable" : p.stationStatus()), false); return 1; }))
                .then(Commands.literal("stop").executes(c -> station(c.getSource(), CinemarrPayloads.StationAction.STOP, CinemarrPayloads.StationType.NONE, false))));
    ...
    private static LiteralArgumentBuilder<CommandSourceStack> operator(String name, CinemarrPayloads.ControlAction action) {
        return Commands.literal(name).requires(s -> s.hasPermission(CinemarrSettings.operatorPermissionLevel())).executes(c -> {
            CinemarrServer.instance().control(c.getSource().getPlayerOrException(), new CinemarrPayloads.ControlRequest(action, -1)); return 1;
        });
    }

    private static int station(CommandSourceStack source, CinemarrPayloads.StationAction action,
                               CinemarrPayloads.StationType type, boolean enabled)
        ...
        player.station(source.getPlayerOrException(), new CinemarrPayloads.StationRequest(
