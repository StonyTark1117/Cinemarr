...
public final class CinemarrPayloads {
    ...
    public record OpenScreen() implements CinemarrMessage {
        public static final ResourceLocation ID = id("open_screen");
    }

    public record ClientHello(int protocolVersion) implements CinemarrMessage {
    ...
    public record ServerHello(int protocolVersion, long serverEpochMs) implements CinemarrMessage {
    ...
    public record TimeSyncRequest(long nonce, long clientSentEpochMs) implements CinemarrMessage {
    ...
    public record TimeSyncResponse(long nonce, long clientSentEpochMs, long serverEpochMs) implements CinemarrMessage {
    ...
    public record BrowseRequest(BrowseKind kind, String query, int page) implements CinemarrMessage {
    ...
    public record BrowseResults(BrowseKind kind, String query, int page, boolean hasMore, List<MediaItem> items) implements CinemarrMessage {
        ...
        public static BrowseResults read(FriendlyByteBuf b) {
            ...
            List<MediaItem> items = value.items().stream().map(CinemarrPayloads::toPayload).toList();
        ...
        public void write(FriendlyByteBuf b) {
            ControlPackets.BROWSE_RESULTS.encode(new MinecraftWireOutput(b), new ControlPackets.BrowseResults(
                    ...
                    items.stream().map(CinemarrPayloads::toCore).toList()));
        }
    }

    public record QueueRequest(ItemKind kind, String key) implements CinemarrMessage {
    ...
    public record ControlRequest(ControlAction action, int index, String expectedKey) implements CinemarrMessage {
    ...
    public record StationRequest(StationAction action, StationType stationType, boolean enabled, long expectedGeneration,
                                 List<StationSeed> seeds) implements CinemarrMessage {
        ...
        public static StationRequest read(FriendlyByteBuf b) {
            ...
            List<StationSeed> seeds = value.seeds().stream().map(CinemarrPayloads::toPayload).toList();
        ...
        public void write(FriendlyByteBuf b) {
            ControlPackets.STATION_REQUEST.encode(new MinecraftWireOutput(b), new ControlPackets.StationRequest(
                    ...
                    enabled, expectedGeneration, seeds.stream().map(CinemarrPayloads::toCore).toList()));
        }
    }

    public record ChunkRequest(UUID sessionId, long requestId, int startIndex, int count) implements CinemarrMessage {
    ...
    public record ChunkAcknowledgement(UUID sessionId, long requestId, int receivedThroughIndex, long bufferedMs) implements CinemarrMessage {
    ...
    public record AudioHealth(UUID sessionId, String state, int recoveryAttempts, int underruns, int receivedChunks, long bufferedMs) implements CinemarrMessage {
    ...
    public record ManifestRequest(boolean forceRebuffer) implements CinemarrMessage {
    ...
    public record AudioManifest(UUID sessionId, String title, String artist, int totalChunks, int firstChunk, long durationMs,
                                long startedAtEpochMs, boolean paused, long pausedPositionMs, String sha256) implements CinemarrMessage {
    ...
    public record AudioChunk(UUID sessionId, long requestId, int index, long startMs, String sha256, byte[] data) implements CinemarrMessage {
    ...
    public record PlaybackState(PlaybackStatus status, String statusMessage, String title, String artist, boolean paused,
                                ...
                                PlaybackOrigin origin, String sourceName, List<QueueEntry> queue) implements CinemarrMessage {
        ...
        public static PlaybackState read(FriendlyByteBuf b) {
            ...
            return new PlaybackState(enumValue(PlaybackStatus.class, value.status()), value.statusMessage(),
                    ...
                    value.sourceName(), value.queue().stream().map(CinemarrPayloads::toPayload).toList());
        }
        public void write(FriendlyByteBuf b) {
            StatePackets.PLAYBACK_STATE.encode(new MinecraftWireOutput(b), new StatePackets.PlaybackState(
                    ...
                    sourceName, queue.stream().map(CinemarrPayloads::toCore).toList()));
        }
    }
    ...
    public record StationState(StationType stationType, boolean active, boolean autoplayEnabled, long generation,
                               ...
                               List<StationSeed> seeds, List<QueueEntry> preview) implements CinemarrMessage {
        ...
        public static StationState read(FriendlyByteBuf b) {
            ...
            return new StationState(enumValue(StationType.class, value.stationType()), value.active(),
                    ...
                    value.capabilityMessage(), value.name(), value.seeds().stream().map(CinemarrPayloads::toPayload).toList(),
                    value.preview().stream().map(CinemarrPayloads::toPayload).toList());
        }
        public void write(FriendlyByteBuf b) {
            StatePackets.STATION_STATE.encode(new MinecraftWireOutput(b), new StatePackets.StationState(
                    ...
                    seeds.stream().map(CinemarrPayloads::toCore).toList(),
                    preview.stream().map(CinemarrPayloads::toCore).toList()));
        }
    }

    public record AdventurePreview(long generation, String message, List<QueueEntry> path) implements CinemarrMessage {
        ...
        public static AdventurePreview read(FriendlyByteBuf b) {
            ...
            return new AdventurePreview(value.generation(), value.message(),
                    value.path().stream().map(CinemarrPayloads::toPayload).toList());
        }
        public void write(FriendlyByteBuf b) {
            StatePackets.ADVENTURE_PREVIEW.encode(new MinecraftWireOutput(b), new StatePackets.AdventurePreview(
                    generation, message, path.stream().map(CinemarrPayloads::toCore).toList()));
        }
    }

    public record ErrorMessage(ErrorCode code, String message) implements CinemarrMessage {
    ...
    public static ResourceLocation idOf(CinemarrMessage message) {
        ...
        throw new IllegalArgumentException("Unknown Cinemarr message type: " + message.getClass().getName());
    }

    public static void write(CinemarrMessage message, FriendlyByteBuf buffer) {
        ...
        else throw new IllegalArgumentException("Unknown Cinemarr message type: " + message.getClass().getName());
    }
