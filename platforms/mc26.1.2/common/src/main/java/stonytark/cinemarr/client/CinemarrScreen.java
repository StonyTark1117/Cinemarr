...
public final class CinemarrScreen extends Screen {
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (requestPending) graphics.centeredText(font, Component.translatable("cinemarr.screen.searching"), width / 2, height / 2, 0xA0D8FF);
        else if (queuePending) graphics.centeredText(font, Component.translatable("cinemarr.screen.queuing"), width / 2, height / 2, 0xA0D8FF);
        else if (playing.status() == CinemarrPayloads.PlaybackStatus.PLEX_OFFLINE && notice.isBlank()) graphics.centeredText(font, Component.translatable("cinemarr.screen.plex_unavailable"), width / 2, height / 2, 0xFF7777);
    }
