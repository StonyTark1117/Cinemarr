...
public final class CinemarrVideoScreen extends Screen {
    ...
    private int acceptanceScreenshotTicks;
    ...
    private void addControls(){
        ...
        widget("display-settings",Button.builder(Component.literal("Display"),b->CinemarrClientUi.showScreen(new Mc26DisplaySettingsScreen(controllerPos,state,this))).bounds(layout.left(),6,68,20).build());
    ...
    @Override public boolean keyPressed(net.minecraft.client.input.KeyEvent event){if(event.key()==257&&search!=null&&search.isFocused()){query=search.getValue().trim();page=rowOffset=0;request();return true;}return super.keyPressed(event);}
    ...
    @Override public void tick(){super.tick();if(acceptanceScreenshotTicks>0&&--acceptanceScreenshotTicks==0&&minecraft!=null)CinemarrVideoUiCapture.capture(minecraft);}
    @Override public void extractRenderState(GuiGraphicsExtractor graphics,int mouseX,int mouseY,float partial){super.extractRenderState(graphics,mouseX,mouseY,partial);graphics.centeredText(font,title,width/2,6,0xffffffff);VideoPackets.SessionState value=state.session(controllerPos);String now=value==null?"No TV state":value.status().name().toLowerCase()+(value.item()==null?"":": "+value.item().title()+"  "+time(CinemarrVideoPlayback.authoritativePositionMsLocal(value))+"/"+time(value.durationMs()));graphics.centeredText(font,trim(now,width-20),width/2,18,0xffa0d8ff);String notice=feedback.message();if(!notice.isEmpty())graphics.centeredText(font,trim(notice,width-20),width/2,layout.noticeY(),0xffffb36b);}
