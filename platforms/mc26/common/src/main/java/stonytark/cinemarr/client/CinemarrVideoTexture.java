...
public final class CinemarrVideoTexture implements AutoCloseable {
    ...
    public CinemarrVideoTexture forDisplay(stonytark.cinemarr.core.protocol.VideoPackets.SessionState state) {
        ...
        if (!presented.canRaster(state.screenWidth(),state.screenHeight())) {
            ...
            if (!budgetErrorShown) {
                ...
                if (Minecraft.getInstance().player != null) Minecraft.getInstance().player.sendSystemMessage(net.minecraft.network.chat.Component.literal(message));
            }
            return null;
        }
        budgetErrorShown=false;
    ...
    private void uploadRaw(int frameWidth, int frameHeight, byte[] rgba, boolean nearest) {
        ...
        if (texture == null || width != frameWidth || height != frameHeight) {
            ...
            width = frameWidth;
            height = frameHeight;
            texture = new DynamicTexture(() -> "Cinemarr video frame", new NativeImage(width, height, false));
    ...
    @Override public void close() {
        for(Derived value:derived.values())value.texture.close();derived.clear();source=null;presented.clear();budgetErrorShown=false;pixelRenderType=null;detailedRenderType=null;
        if (texture != null) {
