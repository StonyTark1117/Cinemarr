...
final class CinemarrClientUi {
    ...
    static void showVideoError(String message) {
        ...
        if (Minecraft.getInstance().gui.screen() instanceof stonytark.cinemarr.core.video.DisplaySettingsFeedback screen) screen.showError(message);
    }

    static void openScreen(net.minecraft.client.gui.screens.Screen screen) { Minecraft.getInstance().setScreenAndShow(screen); }
    ...
    static double[] acceptanceCamera() {
        ...
        return new double[]{player.getX(),player.getY()+player.getEyeHeight(),player.getZ(),
                ...
                Minecraft.getInstance().gui.screen()!=null?1:0,Minecraft.getInstance().gui.hud.isHidden()?1:0};
    }
