...
public final class CinemarrClientState {
    ...
    public void tick() {
        ...
        long syncInterval = mediaClockReady() ? STEADY_CLOCK_SYNC_INTERVAL_MS : STARTUP_CLOCK_SYNC_INTERVAL_MS;
    ...
    private void acceptVideoProbe(CinemarrMessage payload) {
        ...
        if (payload instanceof VideoPayloads.SessionState value) {
            ...
            Cinemarr.LOGGER.info("Acceptance video session: controller={} session={} generation={} status={} item={} positionMs={} canControl={}", state.controllerPos(), state.sessionId(), state.generation(), state.status(), state.item() == null ? "" : state.item().key(), state.positionMs(), state.canControl());
            ...
            if (!acceptanceVideoTuneSent) {
                ...
                CinemarrVideoClientState.INSTANCE.command(new VideoPackets.SessionCommand(VideoPackets.SessionAction.TUNE, state.controllerPos(), "", "", "cinemarr-acceptance", PresentationMode.FIT, state.generation(), 0, -1, -1));
            ...
            if (ProtocolLimits.videoProbeLeader() && acceptanceVideoController != 0 && !acceptanceVideoBrowseRequested && !value.value().libraries().isEmpty()) {
            ...
            if (ProtocolLimits.videoProbeLeader() && !acceptanceVideoPlaySent && acceptanceVideoController != 0 && !value.value().items().isEmpty()) {
                ...
                CinemarrVideoClientState.INSTANCE.command(new VideoPackets.SessionCommand(VideoPackets.SessionAction.PLAY, acceptanceVideoController, value.value().libraryId(), value.value().items().get(0).key(), "", PresentationMode.FIT, state.generation(), 0, -1, -1));
            ...
            Cinemarr.LOGGER.info("Acceptance video manifest: session={} generation={} dimensions={}x{} segments={}", value.value().sessionId(), value.value().generation(), value.value().width(), value.value().height(), value.value().segments().size());
        }
    }
