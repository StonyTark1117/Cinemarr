...
public final class AcceptanceSegmentPeer {
    ...
    public void session(UUID id, long value, boolean owner, boolean playing) {
        session(id == null || value < 0 ? null : new VideoStreamIdentity(id, value, id, value), owner, playing);
    }
    public void session(VideoStreamIdentity value, boolean owner, boolean playing) {
        if (!ProtocolLimits.segmentPressurePeerEnabled()) return;
        boolean eligible = !owner && playing && value != null && !value.streamId().equals(new UUID(0, 0));
        if (running && (!eligible || !value.equals(identity))) {
            unexpectedErrors++; running = false; finished = true;
        }
        if (!eligible) { identity = null; session = null; return; }
        identity = value; session = value.streamId(); generation = value.streamGeneration();
    }
    ...
    public void tick(long now, Consumer<CinemarrMessage> sender, Consumer<String> logger) {
        ...
        if (running) {
            ...
            if (elapsed >= 40_000L) { running = false; finished = true; }
                ...
                for (int i = 0; i < 10; i++) sender.accept(new VideoPackets.SegmentRequest(identity, ++requests, 0, 0, 8));
                nextSend = now + 100L;
            }
        }
    ...
    public void chunk(VideoPackets.SegmentChunk value, Consumer<CinemarrMessage> sender) {
        ...
        if (value.chunkIndex() == Math.min(7, value.totalChunks() - 1)) {
            sender.accept(new VideoPackets.SegmentAcknowledgement(identity, value.requestId(), 0, value.chunkIndex(), 0));
            acknowledgements++;
        }
    }
    ...
    public void reset() {
        identity = null; session = null; generation = 0; startedAt = -1; nextSend = lastReport = elapsed = 0;
