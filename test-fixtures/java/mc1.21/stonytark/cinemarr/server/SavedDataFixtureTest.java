...
class SavedDataFixtureTest {
    ...
    private static CinemarrSavedData roundTrip(String fixture) throws Exception {
        CinemarrSavedData migrated = CinemarrSavedData.load(read(fixture), null);
        CompoundTag canonical = migrated.save(new CompoundTag(), null);
        assertEquals(4, canonical.getInt("schemaVersion"), fixture);
        CinemarrSavedData restored = CinemarrSavedData.load(canonical, null);
    ...
    private static void assertSchemaOne(CinemarrSavedData state) {
        assertEquals("v1-current", state.current().key()); assertEquals(CinemarrPayloads.PlaybackOrigin.MANUAL, state.currentOrigin());
    ...
    private static void assertSchemaTwo(CinemarrSavedData state) {
        assertEquals("v2-current", state.current().key()); assertEquals(CinemarrPayloads.PlaybackOrigin.STATION, state.currentOrigin());
    ...
    private static void assertSchemaThree(CinemarrSavedData state) {
        assertEquals("v3-current", state.current().key()); assertEquals(CinemarrPayloads.PlaybackOrigin.ADVENTURE, state.currentOrigin());
        assertEquals("Sonic Adventure: Fixture Route", state.currentSourceName());
        assertEquals(CinemarrPayloads.StationType.SONIC_ADVENTURE, state.station().type()); assertEquals(17, state.station().generation());
