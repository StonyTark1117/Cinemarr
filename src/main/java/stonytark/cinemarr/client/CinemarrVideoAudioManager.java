...
public final class CinemarrVideoAudioManager {
    ...
    public void tick(CinemarrVideoPlaybackManager playback,CinemarrVideoClientState state){
        ...
        for(CinemarrVideoClientState.StreamState stream:state.streamStates()){
            ...
            stonytark.cinemarr.core.protocol.VideoPackets.SessionState nearest=televisions.stream().min(java.util.Comparator.comparingDouble(tv->CinemarrVideoAudio.nearestScreenPoint(tv,listener).distanceToSqr(listener))).orElse(televisions.getFirst());
