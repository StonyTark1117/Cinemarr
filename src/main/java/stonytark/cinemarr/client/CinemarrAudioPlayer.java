...
import stonytark.cinemarr.core.client.ChunkWindowTracker;
import stonytark.cinemarr.core.client.ClockSynchronizer;
import stonytark.cinemarr.core.client.DriftPolicy;
...
public final class CinemarrAudioPlayer {
    ...
    private CinemarrPayloads.AudioManifest manifest;
    ...
    public CinemarrAudioPlayer(ClockSynchronizer clock) { this.clock = clock; }

    public void manifest(CinemarrPayloads.AudioManifest value) {
        ...
        if (manifest == null || !manifest.sessionId().equals(value.sessionId())) {
            ...
            if (CinemarrSettings.enabled()) beginStreaming();
        } else {
            ...
            manifest = value;
            if (timelineChanged && CinemarrSettings.enabled()) rebuffer();
            else if (decoder == null && CinemarrSettings.enabled()) beginStreaming();
        }
        if (channel != null) channel.execute(c -> { if (value.paused()) c.pause(); else c.unpause(); });
    }

    public void chunk(CinemarrPayloads.AudioChunk value) {
        ...
        if (receivedChunks++ == 0) Cinemarr.LOGGER.info("Cinemarr received the first audio chunk");
        if (ProtocolLimits.audioProbeEnabled() && (value.index() == manifest.firstChunk() || value.index() % 8 == 0)) {
            Cinemarr.LOGGER.info("Acceptance audio chunk: index={} request={} bufferedMs={}",
                    value.index(), value.requestId(), decoder.bufferedMillis());
        }
        lastAudioDataMs = System.currentTimeMillis();
        window.received(value.requestId(), value.index()).ifPresent(ack -> CinemarrNetwork.sendToServer(
                new CinemarrPayloads.ChunkAcknowledgement(manifest.sessionId(), ack.requestId(), ack.receivedThroughIndex(), decoder.bufferedMillis())));
    }
    ...
    public void tick() {
        ...
        window.request(now, decoder.bufferedMillis(), StreamingMp3Decoder.MAX_BUFFERED_MS).ifPresent(request -> {
            if (request.id() == 1) Cinemarr.LOGGER.info("Cinemarr requested the initial audio chunk window");
            ...
            CinemarrNetwork.sendToServer(new CinemarrPayloads.ChunkRequest(manifest.sessionId(), request.id(), request.startIndex(), request.count()));
        });
        Minecraft minecraft = Minecraft.getInstance();
        if (CinemarrSettings.enabled()) minecraft.getMusicManager().stopPlaying();
        long localStart = clock.toLocalTime(manifest.startedAtEpochMs() + Math.max(0, firstChunkStartMs));
        if (!started && !channelStarts.pending() && !manifest.paused() && CinemarrSettings.enabled() && decoder.format() != null && decoder.bufferedMillis() >= START_BUFFER_MS && firstChunkStartMs >= 0 && now >= localStart) {
            startChannel(now);
        }
        if (channel != null) {
            float volume = CinemarrSettings.enabled() ? (float)(CinemarrSettings.volume() * minecraft.options.getSoundSourceVolume(SoundSource.MUSIC)) : 0;
    ...
    public void ensureStarted() { if (manifest != null && decoder == null && CinemarrSettings.enabled()) beginStreaming(); }
    public void listeningChanged() {
        if (!CinemarrSettings.enabled()) {
            resetAudio();
        } else if (manifest != null) {
            resetAudio();
            recovering = true;
            CinemarrNetwork.sendToServer(new CinemarrPayloads.ManifestRequest(true));
        }
    }
    public boolean active() { return manifest != null && CinemarrSettings.enabled(); }
    public AudioPlaybackState state() {
        if (!CinemarrSettings.enabled()) return AudioPlaybackState.DISABLED;
    ...
    public void retry() {
        if (manifest == null || !CinemarrSettings.enabled()) return;
        recoveryAttempts = 0;
    ...
    private void requestRebuffer(String reason) {
        ...
        recovering = true;
        Cinemarr.LOGGER.warn("Cinemarr audio recovery attempt {}/{}: {}", recoveryAttempts, MAX_RECOVERY_ATTEMPTS, reason);
        if (ProtocolLimits.audioProbeEnabled()) {
            Cinemarr.LOGGER.info("Acceptance audio state: RECOVERING reason={}", reason);
        }
        resetAudio();
        CinemarrNetwork.sendToServer(new CinemarrPayloads.ManifestRequest(true));
    }
    ...
    public void acceptanceClockDrift() {
        ...
        lastCorrectionMs = 0;
        Cinemarr.LOGGER.info("Acceptance clock drift injected beyond {} ms", DRIFT_REBUFFER_MS);
    }
    ...
    public void audioEngineReloaded() { if (manifest != null) { resetAudio(); if (CinemarrSettings.enabled()) { recovering = true; CinemarrNetwork.sendToServer(new CinemarrPayloads.ManifestRequest(true)); } } }
    ...
    private void sendHealthIfNeeded(long now) {
        ...
        if (!state.equals(lastHealthState) || now - lastHealthSentMs >= 5_000) {
            ...
            CinemarrNetwork.sendToServer(new CinemarrPayloads.AudioHealth(manifest.sessionId(), state, recoveryAttempts, underruns, receivedChunks, buffered));
