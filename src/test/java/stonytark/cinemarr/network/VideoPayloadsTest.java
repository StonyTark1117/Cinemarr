...
class VideoPayloadsTest {
    @Test void protocolElevenIsAdvertisedAndSessionStateRoundTripsThroughNeoForge(){
        assertEquals(11,ProtocolLimits.VERSION);UUID tv=UUID.randomUUID(),session=UUID.randomUUID();
        VideoPackets.SessionState state=new VideoPackets.SessionState(tv,99L,session,2,VideoPackets.SessionStatus.PLAYING,
                new VideoMediaItem(MediaKind.MOVIE,"1","Movie","","PG",0,90_000),1_000,90_000,false,PresentationMode.FIT,17,11,new byte[]{3,4},ScreenFacing.NORTH,42,-8,10,List.of(),-1,-1,5_000,true,"ok").withTimeline(UUID.randomUUID(), 41);
        VideoPayloads.SessionState decoded=roundTrip(VideoPayloads.SessionState.CODEC,new VideoPayloads.SessionState(state));
        assertEquals(state.identity(),decoded.value().identity());assertEquals(tv,decoded.value().televisionId());assertEquals("Movie",decoded.value().item().title());assertArrayEquals(new byte[]{3,4},decoded.value().visibilityMask());
    ...
    @Test void browseAndManifestRoundTrip(){
        ...
        UUID session=UUID.randomUUID();VideoPackets.SegmentManifest manifest=new VideoPackets.SegmentManifest(new VideoStreamIdentity(UUID.randomUUID(),41,session,1),640,360,"mpegts","h264","aac",1,false,List.of(new VideoPackets.SegmentDescriptor(0,0,1000,true,0,"")));
        VideoPackets.SegmentManifest decoded=roundTrip(VideoPayloads.SegmentManifest.CODEC,new VideoPayloads.SegmentManifest(manifest)).value();
        assertEquals(manifest.identity(),decoded.identity());assertEquals(640,decoded.width());
    }
