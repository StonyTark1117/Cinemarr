...
# Main full-video profiles must exercise display commands. Dedicated fault/terminal
# supplements retain their own scenario and run after the main feature case.
display_feature_gate=${CINEMARR_DISPLAY_FEATURE_GATE:-$video_control_gate}
if [[ "$video_display_gate" == true || "$video_terminal_gate" == true || "$video_pressure_gate" == true || "$video_adverse_network_gate" == true ]]; then display_feature_gate=false; fi
if [[ "$display_feature_gate" != true && "$display_feature_gate" != false ]]; then
  echo 'CINEMARR_DISPLAY_FEATURE_GATE must be true or false' >&2; exit 2
fi
if [[ "$display_feature_gate" == true && "$video_control_gate" != true ]]; then
  echo 'Display feature acceptance requires the full video control gate' >&2; exit 2
fi
...
start_audio_client() {
  ...
  if [[ "$video_client_gate" == "true" ]]; then
    ...
    if [[ "$video_display_gate" == true || "$display_feature_gate" == true ]]; then java_options+=' -Dcinemarr.acceptance.displayProbe=true'; fi
    if [[ "$display_feature_gate" == true ]]; then java_options+=' -Dcinemarr.acceptance.displayFeatureProbe=true'; fi
  ...
  if [[ "$video_display_gate" == true ]]; then
    # Color calibration requires an unmodified framebuffer. Vanilla's vignette
    # depends on light at the viewer, so random surrounding terrain can tint
    # one client's block samples. Keys span legacy, 1.20/1.21 and 26.x options.
    printf '%s\n' 'fancyGraphics:false' 'graphicsMode:0' 'vignette:false' >> "$client_dir/options.txt"
  fi
  ...
  if [[ "$label" == '1.7.10-forge' ]]; then
    printf '%s\n' 'clouds:false' >> "$client_dir/options.txt"
  else
    printf '%s\n' 'renderClouds:false' >> "$client_dir/options.txt"
  fi
...
run_two_client_video() {
  ...
  if (( result == 0 )) && [[ "$display_feature_gate" == true ]]; then
    local display_fixture_args=()
    if [[ "$live_plex_gate" != true ]]; then display_fixture_args=(--fixture-state "$fake_plex_state"); fi
    local display_server_args=()
    if [[ "$live_plex_gate" == true ]]; then
      display_server_args=(--discopanel-server-id "$server_id")
    elif [[ "$label" == '1.7.10-forge' ]]; then
      display_server_args=(--server-fifo "$fifo")
    else
      display_server_args=(--rcon-port "$rcon_port" --rcon-password "$rcon_password")
    fi
    python3 "$repo_root/scripts/observe-display-feature.py" \
      --leader-log "$leader_log" --follower-log "$follower_log" \
      --output "$output_root/$label.display-feature" --gate-pid "$$" \
      "${display_fixture_args[@]}" "${display_server_args[@]}" || result=1
    if (( result == 0 )); then wait_for_video_audio_pair_stable "$label" "$leader_pid" "$follower_pid" || result=1; fi
  fi
  ...
  if (( result == 0 )) && [[ "$display_feature_gate" == true && "$video_control_gate" == true && "$video_terminal_gate" != true ]]; then
    python3 "$repo_root/scripts/observe-display-feature.py" \
      --leader-log "$leader_log" --follower-log "$follower_log" \
      --output "$output_root/$label.display-feature-preserved" --gate-pid "$$" \
      --verify-preserved "$output_root/$label.display-feature/result.json" \
      "${display_server_args[@]}" || result=1
  fi
...
# Runs after the first process has saved and exited, while its isolated world and
# configuration are still installed. The parent run_target owns these variables.
run_display_restart_check() {
  local restart_log="$output_root/$label.display-restart.console.log"
  local restart_result="$output_root/$label.display-restart.json"
  local restart_fifo_dir restart_fifo restart_fd restart_pid restart_group='' restart_server=''
  local restart_status=0 deadline
  restart_fifo_dir=$(mktemp -d "$output_root/$label.restart-fifo.XXXXXX")
  ...
  restart_pid=$!
  active_server_pid=$restart_pid
  deadline=$((SECONDS + 180))
  while ! grep -Eq 'Done \([^)]*\)! For help' "$restart_log" 2>/dev/null; do
    if ! kill -0 "$restart_pid" 2>/dev/null || (( SECONDS >= deadline )); then
      echo "$label: display-persistence restart did not become ready" >&2
      restart_status=1
      break
    fi
    sleep 1
  ...
  if [[ -n "$restart_group" ]]; then
    if ! wait_for_group_exit "$restart_group" 60; then
      restart_status=1
      stop_group "$restart_group" TERM
      wait_for_group_exit "$restart_group" 10 || stop_group "$restart_group" KILL
    fi
  elif ! wait_for_process_tree_exit "$restart_pid" 60; then
    restart_status=1
  ...
  exec {restart_fd}>&-
  rm -f -- "$restart_fifo"
  rmdir -- "$restart_fifo_dir"
  if ! grep -Eq 'Stopping (the )?server' "$restart_log" || ! grep -Fq 'Saving players' "$restart_log" \
      || grep -Eiq 'Failed to start the minecraft server|ModLoadingException|Preparing crash report|Encountered an unexpected exception' "$restart_log" \
      || grep -Eiq 'Unable to close Plex video sessions|Unable to stop inactive Plex video session|Timed out draining Plex media lifecycle' "$restart_log" \
      || ss -ltnH "sport = :$port or sport = :$rcon_port" | grep -q .; then
    echo "$label: display-persistence restart did not shut down cleanly" >&2
    restart_status=1
  fi
  if (( restart_status == 0 )); then
    python3 "$repo_root/scripts/check-display-persistence.py" \
      --expected "$output_root/$label.display-feature/result.json" --log "$restart_log" \
      --output "$restart_result" || restart_status=1
  fi
...
run_target() {
  ...
  if [[ "$video_client_gate" == "true" ]]; then
    ...
    if [[ "$video_display_gate" == true || "$display_feature_gate" == true ]]; then server_java_options+=' -Dcinemarr.acceptance.displayProbe=true'; fi
    if [[ "$display_feature_gate" == true ]]; then server_java_options+=' -Dcinemarr.acceptance.displayFeatureProbe=true'; fi
    if [[ "$video_terminal_gate" == true && "$label" == '1.7.10-forge' ]]; then
  ...
  if (( result == 0 )) && [[ "$display_feature_gate" == true && "$video_control_gate" == true \
      && "$video_terminal_gate" != true && "$external_video_client_gate" != true ]]; then
    run_display_restart_check || result=1
  fi
  active_game_port=""
  active_rcon_port=""
