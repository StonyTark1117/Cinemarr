...
if [[ "$video_terminal_gate" == true || "$video_pressure_gate" == true || "$video_adverse_network_gate" == true ]]; then display_feature_gate=false; fi
...
start_audio_client() {
  ...
  if [[ "$video_client_gate" == "true" ]]; then
    ...
    if [[ "$display_feature_gate" == true ]]; then java_options+=' -Dcinemarr.acceptance.displayProbe=true'; fi
...
run_target() {
  ...
  if [[ "$video_client_gate" == "true" ]]; then
    ...
    if [[ "$display_feature_gate" == true ]]; then server_java_options+=' -Dcinemarr.acceptance.displayProbe=true'; fi
    if [[ "$video_terminal_gate" == true && "$label" == '1.7.10-forge' ]]; then
