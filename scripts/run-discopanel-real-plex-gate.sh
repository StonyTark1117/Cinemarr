...
acceptance_overrides=$(jq -c --arg display "$display_gate" '
  ...
  | .environment.JAVA_TOOL_OPTIONS = (((.environment.JAVA_TOOL_OPTIONS // "") + " -Dcinemarr.acceptance.enabled=true -Dcinemarr.acceptance.videoProbe=true -Dcinemarr.acceptance.displayProbe=true" + (if $display == "true" then "" else " -Dcinemarr.acceptance.displayFeatureProbe=true" end)) | ltrimstr(" "))
...
if [[ "$display_gate" == true ]]; then export CINEMARR_DISPLAY_FEATURE_GATE=false; fi
