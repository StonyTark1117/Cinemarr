...
The probe intentionally implements no loader protocol and never sends a Cinemarr
...
def login_payload(protocol: int, name: str) -> bytes:
    ...
    if protocol <= 763:
        ...
        payload.extend(uuid.uuid5(uuid.NAMESPACE_DNS, "cinemarr-login-probe").bytes)
    return bytes(payload)
...
def probe(
    ...
    with socket.create_connection((host, port), timeout=timeout) as raw:
        ...
        connection.send(0, login_payload(protocol, "CinemarrProbe"))
